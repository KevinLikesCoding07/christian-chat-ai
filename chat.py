...
@st.cache_resource
def load_ai_model():
    return pipeline("text-generation", model="unsloth/Phi-3-mini-4k-instruct", torch_dtype="auto", device_map="auto")
...
if user_input := st.chat_input("How can I help or pray for you today my brother/sister?"):
    ...
    system_instruction = (
        ...
        "Provide a bible verse relating to their problems and also make sure to give them a prayer. "
        "Try to use the bible for relevance to their problems. Provide them encouragement too."
    )
    ...
    with st.chat_message("assistant"):
        with st.spinner("Analyzing...."):
            outputs = generator(full_prompt, max_new_tokens=250, do_sample=True, temperature=0.7)
            bot_response = outputs[0]['generated_text'].split("<|assistant|>\n")[-1].strip()
